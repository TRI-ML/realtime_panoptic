...
from realtime_panoptic.layers.group_norm import ONNXAbleGN
...
from realtime_panoptic.models.panoptic_from_dense_box_optimized import PanopticFromDenseBox

class RTPanoNetOpt(torch.nn.Module):
    ...
    def __init__(
        ...
        super(RTPanoNetOpt, self).__init__()
    ...
    def forward(self, images, detection_targets=None, segmentation_targets=None):
        ...
        downsampled_level = images.shape[-1] // semantic_logits.shape[-1]
        interpolated_semantic_logits_padded = F.interpolate(semantic_logits, scale_factor=downsampled_level, mode='bilinear')
        interpolated_semantic_logits = interpolated_semantic_logits_padded[:,:,:images.shape[-2], :images.shape[-1]]
        ...
        boxes = self.panoptic_from_dense_bounding_box.process(
            locations, box_cls, box_regression, centerness, levelness_logits, semantic_logits, [images.shape[-2],
                                                                                                images.shape[-1]]
        )
        instance_masks = [box.get_field("mask") for box in boxes]
        ...
        return instance_masks, interpolated_semantic_logits
...
class PanopticHead(torch.nn.Module):
    ...
    def __init__(
        ...
        for i in range(fcos_num_convs):
            ...
            if fcos_norm == "GN":
                cls_tower.append(ONNXAbleGN(mid_channels // 8, mid_channels))
            elif fcos_norm == "BN":
            ...
            if fcos_norm == "GN":
                bbox_tower.append(ONNXAbleGN(in_channels // 8, in_channels))
            elif fcos_norm == "BN":
